...
export default function BlogCard({
  ...
  const getThemeClass = () => {
    switch (settings.cardStyle) {
      case "slate":
        return "bg-slate-600 dark:bg-slate-800 text-white";
      case "rose":
        return "bg-rose-500 dark:bg-rose-800 text-white";
      case "emerald":
        return "bg-emerald-600 dark:bg-emerald-900 text-white";
      case "midnight":
        return "bg-indigo-900 dark:bg-gray-950 text-white";
      default:
  ...
  const getTitleBgClass = () => {
    switch (settings.cardStyle) {
      ...
      case "rose":
        return "bg-rose-600 dark:bg-rose-900";
      case "emerald":
        return "bg-emerald-700 dark:bg-emerald-950";
      case "midnight":
        return "bg-indigo-950 dark:bg-black";
      default:
        return "bg-cyan-700 dark:bg-cyan-950";
    }
  };
  ...
  const getViewMoreBgClass = () => {
    switch (settings.cardStyle) {
      case "slate":
        return "bg-slate-600 dark:bg-slate-800";
      case "rose":
        return "bg-rose-500 dark:bg-rose-800";
      case "emerald":
        return "bg-emerald-600 dark:bg-emerald-900";
      case "midnight":
        return "bg-indigo-900 dark:bg-gray-950";
      default:
        return "bg-cyan-600 dark:bg-cyan-900";
    }
  };
      ...
      <div key={lineIndex}>
        {line.split(" ").map((word, i) =>
          word.startsWith("#") ? (
            <span
              key={i}
              className="text-cyan-300 dark:text-cyan-400 font-semibold"
            >
              {word}{" "}
            </span>
          ) : (
  ...
  return (
    <div
      className={`w-[90%] max-w-xl rounded-lg overflow-hidden mx-auto wrap-break-word whitespace-normal my-12 ${getThemeClass()}`}
    >
      {/* Title */}
      <div className={`text-center py-1 px-2 relative ${getTitleBgClass()}`}>
        ...
        <h2
          className={`font-semibold text-white wrap-break-word tracking-wide pr-12 break-words ${
      ...
      <div className="relative text-white p-2 tracking-wide leading-snug font-light mx-auto">
        <div
          ...
          className={`cursor-${isOverflowing ? "pointer" : "default"} ${
            isExpanded
              ? ""
              : settings.lineClamp === "1"
                ? "line-clamp-1"
                : settings.lineClamp === "3"
                  ? "line-clamp-3"
                  : settings.lineClamp === "4"
                    ? "line-clamp-4"
                    : "line-clamp-2"
      ...
      <div className="flex flex-row justify-between mx-4 items-center my-2 gap-2">
        ...
        {!change ? (
          <button
            ...
            className={`${variants[variant]} text-white px-3 rounded hover:font-bold ${
              settings.fontSize === "small"
                ? "text-sm md:text-base"
                : settings.fontSize === "large"
                  ? "text-lg md:text-xl"
            ...
            onClick={() => {
              ...
              if (!notification) {
                ...
                if (blog.folderId) {
                  ...
                  addNotification(
                    "Cannot remove: Folder ID not found",
                    "error",
                  );
                }
              }
        ...
        >
          <div
            className={`fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[68%] max-w-sm rounded-xl z-[110] overflow-hidden ${
              settings.cardStyle === "slate"
                ? "bg-slate-700"
                : settings.cardStyle === "rose"
                  ? "bg-rose-600"
                  : settings.cardStyle === "emerald"
                    ? "bg-emerald-700"
                    : settings.cardStyle === "midnight"
                      ? "bg-indigo-950"
                      : "bg-cyan-700"
            }`}
            onClick={(e) => e.stopPropagation()}
          >
            <p className="text-white text-center pt-4 pb-2 font-medium">
              Add to Folder
            </p>
